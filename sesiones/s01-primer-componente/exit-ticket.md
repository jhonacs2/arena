...
**2. Aplicar** — Este botón tiene que quedar deshabilitado cuando `balance` sea menor a 100. Escribí la línea:
...
Respuesta esperada de la 2: `[disabled]="balance < 100"`.

| Lo que escriben | Qué decirles |
|---|---|
| `disabled="balance < 100"` | Sin corchetes es la cadena literal `"balance < 100"`, que es *truthy*: el botón queda deshabilitado **siempre**. Es el mismo error del bloque de predicción. |
| `[disabled]="{{ balance < 100 }}"` | Corchetes **o** llaves, nunca las dos. Los corchetes ya dicen «esto es una expresión». |
| `(disabled)="balance < 100"` | Los paréntesis escuchan eventos. `disabled` no es un evento. |
